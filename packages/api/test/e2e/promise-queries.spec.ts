...
describe.skip('Promise e2e queries', () => {
  ...
  describe('with plain type', async () => {
    ...
    it('queries correct value', async () => {
      ...
      expect(totalIssuance.toNumber()).toEqual(TOTAL_ISSUANCE);
    });

    it('queries correct value at a specified block', async () => {
      const header = await api.rpc.chain.getHeader() as Header;
      const totalIssuanceAt = await api.query.balances.totalIssuance.at(header.hash) as Balance;

      expect(totalIssuanceAt.toNumber()).toEqual(TOTAL_ISSUANCE);
    });

    it('subscribes to query and get correct result', async (done) => {
      api.query.balances.totalIssuance((totalIssuance) => {
        expect(totalIssuance.toNumber()).toEqual(TOTAL_ISSUANCE);
        done();
      });
    });
    ...
    it('queries correct hash', async () => {
      const hash = await api.query.balances.totalIssuance.hash();

      expect(hash).toBeDefined();
    });

    it('gets correct key', async () => {
      const key = await api.query.balances.totalIssuance.key();
    ...
    it('queries correct size', async () => {
      const size = await api.query.balances.totalIssuance.size();
  ...
  describe('with map type', async () => {
    ...
    it('subscribes to query and get correct result', async (done) => {
      ...
      api.query.balances.freeBalance(keyring.alice.address(), (balanceSubscribed) => {
    ...
    it('gets correct key', async () => {
      // assume the account Alice is only used in test(the balance of Alice does not change in this test case)
      const key = await api.query.balances.freeBalance.key(keyring.alice.address());
  ...
  describe('with double map type', async () => {
    ...
    it('subscribes to query and get correct result', async (done) => {
      api.query.system.eventTopics(KEY1, KEY2, (eventTopicsAt) => {
        expect(eventTopicsAt.toJSON()).toEqual([]);
        done();
      });
    });
    ...
    it('gets correct key', async () => {
      const key = await api.query.system.eventTopics.key(KEY1, KEY2);
